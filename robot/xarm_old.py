...
def GUI(robot):
    def move_joint_fn_generator(joint_name):
        def foo(angle_deg):
            angle_rad = int(angle_deg) * np.pi / 180.
            robot.set_joint_angle(joint_name, angle_rad)
            robot.go()
        return foo
    ...
    window = tk.Tk()
    heading = tk.Label(text=str(robot)+" GUI")
    heading.pack()
    ...
    for i, joint_name in enumerate(robot.joint_names):
        row_frame = tk.Frame(master=main_frame, width=W, height=H//6, borderwidth=1)
        ...
        lbl_joint = tk.Label(master=col_frame_left, text=joint_name)
        lbl_joint.pack()

        move_joint_fn = move_joint_fn_generator(joint_name)
        joint_limits = 180*robot.JOINT_LIMITS[i]/np.pi
        scl_joint = tk.Scale(master=col_frame_right,
                             from_=joint_limits[0], to=joint_limits[1],
                             orient=tk.HORIZONTAL,
                             command=move_joint_fn)
        scl_joint.set(int(robot.HOME[i]*180/np.pi))
        scl_joint.pack()

    window.mainloop()

def get_joint_limits(arm):
    joints = ['base','shoulder','elbow', 'wristRotation', 'wrist', 'gripper']
